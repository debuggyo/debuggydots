        ...
        string = string + f"""
                  (button :class 'notif'
                   (box :orientation 'horizontal' :space-evenly false
                      (image :image-width 64 :image-height 64 :path '{item.icon or ''}')
                      (box :orientation 'vertical'
                        (label :class 'title' :width 250 :wrap true :xalign 0 :text '{item.summary or ''}')
                        (label :class 'text' :width 250 :wrap true :xalign 0 :text '{item.body or ''}')
                  )))
                  """
